...
%     SpoofedSUIndex=tmp.SpoofedSUIndex;
    ...
    A=GetAdjacencyMatrix(FalseCoordinate,BeaconRange);%output r tells you which nodes of p belong to the same connected component
%     figure(1)
%     %p_0=0.01*2*rand(SUNumber,1);
%     p_0=0.01.*ones(SUNumber,1);
...
%
    ...
    while(time<PresetTime)%If no more nodes can be verified
       ...
       BeaconControl=(rand(1,LastNumChecked)<=BeaconProbability);
       for i=1:SUNumber
